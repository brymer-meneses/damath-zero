...
auto MCTS::compute_puct_score(NodeId parent_id, NodeId child_id) const -> f64 {
  ...
  auto pb_c =
      std::log((parent.visit_count + config_.c_base + 1.0) / config_.c_base) + config_.c_init;
...
auto MCTS::select_highest_visit_count(NodeId parent_id, i32 game_history_size) const -> NodeId {
  const auto& parent = nodes_.get(parent_id);
  assert(parent.is_expanded());

  if (game_history_size < config_.num_sampling_moves) {
      auto visit_counts = parent.children | std::views::transform([this](NodeId n) {
